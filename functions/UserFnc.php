...
function User($item)
{
	global $_hcwsms, $DefaultSyear;
	if (!$_SESSION['UserSyear'])
		$_SESSION['UserSyear'] = $DefaultSyear;

	if (!$_hcwsms['User'] || $_SESSION['UserSyear'] != $_hcwsms['User'][1]['SYEAR']) {
		if ($_SESSION['STAFF_ID']) {
			...
			$_hcwsms['User'] = DBGet(DBQuery($sql));
			...
			if (count($_hcwsms['User']) == 0) {
				...
				$_hcwsms['User'] = DBGet(DBQuery($sql));
				$_SESSION['UserSchool'] = $_hcwsms['User'][1]['SCHOOL_ID'];
			} else {
				$_SESSION['UserSchool'] = $_hcwsms['User'][1]['SCHOOL_ID'];
			}
		} else {
...
function Preferences($item, $program = 'Preferences')
{
	...
	if ($_SESSION['STAFF_ID'] && !$_hcwsms['Preferences'][$program]) {
		...
		$_hcwsms['Preferences'][$program] = DBGet($QI, array(), array('TITLE'));
	}
	...
	if (!isset($_hcwsms['Preferences'][$program][$item][1]['VALUE']))
		$_hcwsms['Preferences'][$program][$item][1]['VALUE'] = $defaults[$item];

	if ($_SESSION['STAFF_ID'] && User('PROFILE') == 'parent' || $_SESSION['STUDENT_ID'])
		$_hcwsms['Preferences'][$program]['SEARCH'][1]['VALUE'] = 'N';
	if ($program == 'Gradebook') {
		if ($item == 'ANOMALOUS_MAX') {
			$arr = explode('_', $_hcwsms['Preferences'][$program][$item][1]['VALUE']);
			return $arr[0];
		} else
			return rtrim($_hcwsms['Preferences'][$program][$item][1]['VALUE'], '_' . UserCoursePeriod());
	} else
		return $_hcwsms['Preferences'][$program][$item][1]['VALUE'];
}
