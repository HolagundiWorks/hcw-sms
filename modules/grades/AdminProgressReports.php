...
if(!$_REQUEST['modfunc'])
{
	...
	{
		echo "<FORM id=F1 action=ForExport.php?modname=".strip_tags(trim($_REQUEST[modname]))."&modfunc=save&include_inactive=".strip_tags(trim($_REQUEST['include_inactive']))."&_hcwsms_PDF=true method=POST target=_blank>";
...
//	return '<INPUT type=checkbox name=st_arr[] value='.$value.' checked>';
        ...
        return "<input  class=fd name=unused[$THIS_RET[STUDENT_ID]] value=" . $THIS_RET['STUDENT_ID'] . " type='checkbox' id=$THIS_RET[STUDENT_ID] onClick='setHiddenCheckboxStudents(\"st_arr[$THIS_RET[STUDENT_ID]]\",this,$THIS_RET[STUDENT_ID]);' />";
